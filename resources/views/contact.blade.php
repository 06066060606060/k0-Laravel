     ...
     <div data-barba="container">
         ...
         @if (session('Message_envoye'))
             <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 3000, PopupUser())" class="pt-1 pr-4">
                 <div id="popmenu" class="px-4 py-2 text-xs btnmenu text-emerald-500">&zwnj; {{__('contact.ct1')}}</div>
             </div>
         @endif
         @if (session('already_send'))
             <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 3000, PopupUser())" class="pt-1 pr-4">
                 <div id="popmenu" class="px-4 py-2 text-xs btnmenu text-emerald-500">&zwnj; {{__('contact.ct2')}}</div>
             </div>
         @endif
                 ...
                 <section class="relative text-gray-600 body-font">
                     <div class="container px-5 py-8 mx-auto">
                         <div class="flex flex-col w-full mb-4 text-center">
                             <h1 class="mb-4 text-4xl font-bold text-gray-100 md:text-5xl title-font">{{__('contact.ct3')}}</h1>
                         </div>
                         <div class="mx-auto lg:w-1/2 md:w-2/3">
                             <form class="flex flex-wrap -m-2" method="post" action="contactmail">
                                 ...
                                 <div class="w-1/2 p-2">
                                     <div class="relative">
                                         <label for="email" class="text-sm leading-7 text-gray-200">{{__('contact.ct6')}}</label>
                                         <input @if (backpack_auth()->check())
                                 ...
                                 <div class="w-full p-2">
                                     <div class="relative">
                                         <label for="message" class="text-sm leading-7 text-gray-200">{{__('contact.ct7')}}</label>
                                 ...
                                 <div class="flex justify-center w-full p-2">
                                     <button type="submit"
                                         ...
                                         <span class="relative">{{__('contact.ct8')}}</span>
                                     </button>
                                 </div>
                             </form>
                         </div>
